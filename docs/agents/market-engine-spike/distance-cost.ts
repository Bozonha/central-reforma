 ...
 *
 * Implementa, de forma determinística (sem LLM, conforme seção 39 do
 * escopo mestre), a ideia central pedida: a busca de material não deve se
 * limitar a grandes lojas/marketplaces — deve incluir pequenas lojas locais
 * da região do usuário, e a distância até essas lojas é um CUSTO que pode
 * tornar uma oferta mais barata, na prática, inviável.
 *
 * Isto estende o "Motor de Logística" já previsto na seção 16 do documento
 * mestre (custo do produto + frete + combustível + pedágio + estacionamento
 * = custo efetivo), aplicando-o também a lojas locais descobertas por
 * proximidade, não só a ofertas de e-commerce com frete informado.
 */
